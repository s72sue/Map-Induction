...
def get_human_data(results_folder="./human_experiments/exp1_data/"):
    ...
    for hdict in dicts:
        ...
        for tmap_index in range(6):
            for r in range(1):
                ...
                # print("num_rewards: "+str(num_rewards))
        ...
        for m in results.datas.keys():
            her.datas[m] = {}
            her.datas[m][hdict["ID"][0]] = copy.deepcopy(results.datas[m][hdict["ID"][0]])
        with open("./notebooks/human_experiment_results_{}.pkl".format(hdict["ID"][0]), 'wb') as handle:
            pickle.dump(her, handle)

        # with open('./notebooks/human_{}_results_dlos.pkl'.format(exp), 'wb') as handle:
