...
function search(p::POMCPPlanner, b, t::POMCPTree, info::Dict)
    ...
    for i in 1:p.solver.tree_queries
        ...
        if !POMDPs.isterminal(p.problem, s)
            r = simulate(p, s, POMCPObsNode(t, 1), p.solver.max_depth)
            all_terminal = false
        end
    end
...
function simulate(p::POMCPPlanner, s, hnode::POMCPObsNode, steps::Int)
    ...
    for node in t.children[h]
        n = t.n[node]
        if n == 0 && ltn <= 0.0
            criterion_value = t.v[node] # If no nodes have been sampled at all
        elseif n == 0 && t.v[node] == -Inf
            criterion_value = Inf # Sample this node if it has never been sampled
        elseif t.terminalactionnode[node] == 1
            criterion_value = -Inf
        else
    ...
    if(isterminal(p.problem, sp))
        t.terminalactionnode[ha] = 1
    end
