...
function get_action_offset(y::Int64, x::Int64, d::Int64, a::Symbol)
    ...
    elseif a == :forward10
        return (get_range(y, y+dy*10), get_range(x, x+dx*10), d)
...
function get_state_observation_mask(observed_state::GridWorldState, state_grid::Matrix{Int64}, observation_mode::Symbol, ay::Int64, ax::Int64, d::Int64)
...
function find_reachable_unobserved(observation, ay, ax)
    # println("find_reachable_unobserved")
...
# function fill_obsmap(observation, ay, ax)
#     # println("Recalculating obs rewardmap!")
#     sy, sx = size(observation)
#     new_state = zeros(sy, sx)
#     oy, ox = find_reachable_unobserved(observation, ay, ax)
#     if(oy > 0)
#         start = (oy, ox, 0)
#         Q = [start]
#         explored = Dict()
...
#         end
#     end
#     return new_state, GridWorldObs(oy, ox)
# end
...
function grid_tf(hyps::Vector{Matrix{Int64}}, reward_maps::Vector{Vector{Matrix{Float64}}}, s::GridWorldState, os::GridWorldState, a::Symbol, size_y::Int64, size_x::Int64, optimism::Float64)
    ...
    # println("New probs")
    # println(new_probs, "--", length(hyps), "--", length(s.reward_status[1]))

    if(!isnan(sum(new_probs)) && sum(new_probs) > 0.0)
        for hypi in 1:length(hyps)
            max_reward_map = 0
            # println(length(s.reward_status[hypi]),"--", length(reward_maps[hypi]))
            # @assert length(s.reward_status[hypi]) == length(reward_maps[hypi])
    ...
    # print(aux_r)
    # println("Reward map r: ",aux_r)
    aux_r += s.obs_reward_map[offy, offx] * optimism
    # println("Obs map r: ",obs_reward_map[offy, offx] * optimism)
    ...
    # for obs_reward_key_i in s.obs_reward_key
    #     if(os_grid[obs_reward_key_i.ay, obs_reward_key_i.ax] != get_symbol(:unobserved))
    #         aux_r += s.obs_reward_map[obs_reward_key_i.ay, obs_reward_key_i.ax] * optimism
    #         new_obs_reward_map, new_obs_reward_key = fill_obsmap(os_grid)
    #         break
    #     end
    # end
...
function update_observation_from_state(os::GridWorldState, s_grid::Matrix{Int64}, observation_mode::Symbol)
    oscopy = deepcopy(os)
    ay, ax, d = get_agent_loc(oscopy)
    mask = get_state_observation_mask(oscopy, s_grid, observation_mode, ay, ax, d)
...
function POMDPs.gen(m::GridWorld, s::GridWorldState, a::Symbol, rng) 
    ...
    hyp_state = GridWorldState(hyp_grid, s.ay, s.ax, copy(s.d), copy(s.done), deepcopy(s.probs), s.reward_count, s.reward_status)
    ...
    sp = update_observation_from_state(new_observed_state, new_hypstate.grid, m.observation_mode)

    unobserved_count = count_unobserved(sp.grid)
    prev_unobserved_count = count_unobserved(s.grid)
    sp = GridWorldState(sp.grid, sp.ay, sp.ax, sp.d, sp.done, sp.probs, sp.reward_count, sp.reward_status)

    observed_reward = (prev_unobserved_count-unobserved_count)*m.optimism
    return (sp=sp, o=deepcopy(sp), r=r+observed_reward)
end
...
# POMDPs.actions(m::Union{GridWorld, GridWorldSSP}) = [:forward, :backward, :left, :right]
...
function init_observed_state(state_grid, observation_mode)
    sy, sx = size(state_grid)
    ...
    observed_state = update_observation_from_state(get_unobserved(sy, sx, ay, ax, d), state.grid, Symbol(observation_mode))
    return observed_state.grid
end

function next_state(state_grid, observed_state, action, observation_mode, optimism)
    sy, sx = size(state_grid)
    state_grid_matrix = convert(Matrix{Int64}, state_grid)
    ay, ax, d = get_agent_loc(state_grid_matrix)
    ...
    observed_state = update_observation_from_state(next_observed_state, next_state.grid, Symbol(observation_mode))
...
function step_pomcp_ssp(observed_state, 
    ...
    for (s, a, sp, o, r) in stepthrough(pomdp, planner, "s,a,sp,o,r", max_steps=1)
        ...
        push!(actions_to_take, a)
        if(new_num_unobserved != num_unobserved || r>=1.0)
            println("New Num Unobserved: ")
            println(new_num_unobserved)
            println(num_unobserved)
            break
        end
    end
    tree_parts = get_tree_parts(info[:tree])
